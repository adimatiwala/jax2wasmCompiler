(module
  ...
  (func $main (export "main") (param $p0 f32) (param $p1 f32) (result f32)
    (local $tanh.1 f32)
    (local $ROOT %log.1 f32)
    (local $ROOT %tanh_log_fusion f32)
    (f32.store (i32.const 2096) (local.get $p0))
    (f32.store (i32.const 2048) (local.get $p1))
    (local.set $tanh.1 (f32.const 0.0)) ;; TODO: Implement tanh(%param_0.1),
    (local.set $ROOT %log.1 (f32.const 0.0)) ;; TODO: Implement log(%tanh.1),
    (local.set $ROOT %tanh_log_fusion (f32.const 0.0)) ;; TODO: Implement fusion(%Arg_0.1),
    (local.get $ROOT %tanh_log_fusion)
  )
)